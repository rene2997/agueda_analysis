# fuzzer/coverage_tracker.py (modified, for interfacing with Java instrumentation)
...
class CoverageTracker:
    def __init__(self, config):
        ...
        # Initialize a global coverage bitmap, using a bytearray is more efficient
        # The initial value of all positions is 0
        self.global_coverage_map = bytearray(self.config.coverage_map_size)

        # AFL-style hit count classification table. Maps the original count value to a representative "bucket".
        # 0 -> 0, 1 -> 1, 2 -> 2, 3 -> 3, 4-7 -> 4, 8-15 -> 5, 16-31 -> 6, 32-127 -> 7, 128+ -> 8
        self.hit_count_buckets = self._initialize_buckets()

    def _initialize_buckets(self):
        """Pre-calculate the corresponding bucket for each count value from 0-255 to avoid repeated calculations"""
    ...
    def track_execution(self, java_runner, input_data: int) -> Tuple[bool, Optional[str]]:
        """
        Call the Java program, track coverage, and capture exceptions
        :param java_runner: JavaRunner instance (responsible for calling the Java program)
        :param input_data: Test case input
        :return: (has_new_coverage: whether a new branch is covered, error_msg: exception information (None if none))
        """
        # 1. Call the Java program to get the coverage and exceptions for this execution
        ...
        if not coverage_data or "covered_branches" not in coverage_data:
            return False, error_msg  # No coverage data, considered as no new coverage

        # 2. Extract the new branches from this execution
        current_branches = set(coverage_data["covered_branches"])
        # 3. Determine if any new branches have been covered
        has_new_coverage = not current_branches.issubset(self.covered_branches)
        if has_new_coverage:
            self.covered_branches.update(current_branches)  # Update the global covered branches
    ...
    def track_execution2(self, java_runner, new_input):
        ...
        for i in range(self.config.coverage_map_size):
            ...
            if current_run_map[i] != 0:
                ...
                # If the current "bucket" is larger than the global "bucket", it means a new behavior has been discovered
    ...
    def get_coverage_stats2(self):
        """Calculate the total number of covered edges"""
    ...
    def reset(self):
        """Reset coverage (for use when re-execution is required)"""
